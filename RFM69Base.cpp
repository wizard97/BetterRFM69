#include <SPI.h>
#include "RFM69Base.h"
#include "RFM69Baseregisters.h"

volatile uint8_t RFM69Base::DATA[RF69_MAX_DATA_LEN];
volatile uint8_t RFM69Base::_mode;        // current transceiver state
volatile uint8_t RFM69Base::PAYLOADLEN;
volatile int16_t RFM69Base::RSSI;          // most accurate RSSI during reception (closest to the reception)
volatile bool RFM69Base::_inISR;
RFM69Base* RFM69Base::selfPointer;


RFM69Base::RFM69Base(uint8_t slaveSelectPin, uint8_t interruptPin, bool isRFM69HW, uint8_t interruptNum)
{
...
bool RFM69Base::initialize(uint8_t freqBand)
{
    const uint8_t CONFIG[][2] =
    {
      ...
      // looks like PA1 and PA2 are not implemented on RFM69BaseW, hence the max output power is 13dBm
      // +17dBm and +20dBm are possible on RFM69BaseHW
    ...
    setHighPower(_isRFM69HW); // called regardless if it's a RFM69BaseW or RFM69BaseHW
    ...
    if (millis()-start >= timeout)
      return false;
    _inISR = false;
    attachInterrupt(_interruptNum, RFM69Base::isr0, RISING);
...
uint32_t RFM69Base::getFrequency()
{
...
void RFM69Base::setSyncAddr(const uint8_t *addr, uint8_t len)
{
    if (!len) {
...
void RFM69Base::setPayloadLength(uint8_t len, bool variable)
{
    if (len > 255)
        return;
...
void RFM69Base::setFrequency(uint32_t freqHz)
{
...
void RFM69Base::setFrequency(uint8_t freq[3])
{
...
void RFM69Base::setMode(uint8_t newMode)
{
  ...
  switch (newMode) {
    case RF69_MODE_TX:
      ...
      if (_isRFM69BaseHW) setHighPowerRegs(true);
      break;
    case RF69_MODE_RX:
      writeReg(REG_OPMODE, (readReg(REG_OPMODE) & 0xE3) | RF_OPMODE_RECEIVER);
      if (_isRFM69BaseHW) setHighPowerRegs(false);
      break;
...
//       - for RFM69BaseW the range is from 0-31 [-18dBm to 13dBm] (PA0 only on RFIO pin)
//       - for RFM69BaseHW the range is from 0-31 [5dBm to 20dBm]  (PA1 & PA2 on PA_BOOST pin & high Power PA settings - see section 3.3.7 in datasheet, p22)
void RFM69Base::setPowerLevel(uint8_t powerLevel)
{
  _powerLevel = (powerLevel > 31 ? 31 : powerLevel);
  if (_isRFM69BaseHW) _powerLevel /= 2;
...
void RFM69Base::send(const void* buffer, uint8_t bufferSize)
{
...
// internal function
void RFM69Base::isr0() { _inISR = true; selfPointer->interruptHandler(); _inISR = false; }



// internal function - interrupt gets called when a packet is received
void RFM69Base::interruptHandler() {
  //pinMode(4, OUTPUT);
  //digitalWrite(4, 1);
...
void RFM69Base::encrypt(const char* key) {
  setMode(RF69_MODE_STANDBY);
  if (key != 0)
  {
    select();
...
int16_t RFM69Base::readRSSI(bool forceTrigger) {
  int16_t rssi = 0;
  if (forceTrigger)
  {
...
// select the RFM69Base transceiver (save SPI settings, set CS low)
void RFM69Base::select() {
  ...
  _SPSR = SPSR;
#endif
  // set RFM69Base SPI settings
  SPI.setDataMode(SPI_MODE0);
  SPI.setBitOrder(MSBFIRST);
  SPI.setClockDivider(SPI_CLOCK_DIV4); // decided to slow down from DIV2 after SPI stalling in some instances, especially visible on mega1284p when RFM69Base and FLASH chip both present
  digitalWrite(_slaveSelectPin, LOW);
}

// unselect the RFM69Base transceiver (set CS high, restore SPI settings)
void RFM69Base::unselect() {
  digitalWrite(_slaveSelectPin, HIGH);
  // restore SPI settings to what they were before talking to RFM69Base
#if defined (SPCR) && defined (SPSR)
  SPCR = _SPCR;
  SPSR = _SPSR;
#endif
...
// for RFM69BaseHW only: you must call setHighPower(true) after initialize() or else transmission won't work
void RFM69Base::setHighPower(bool onOff) {
  _isRFM69BaseHW = onOff;
  writeReg(REG_OCP, _isRFM69BaseHW ? RF_OCP_OFF : RF_OCP_ON);
  if (_isRFM69BaseHW) // turning ON
...
// internal function
void RFM69Base::setHighPowerRegs(bool onOff) {
  writeReg(REG_TESTPA1, onOff ? 0x5D : 0x55);
  writeReg(REG_TESTPA2, onOff ? 0x7C : 0x70);
}

// set the slave select (CS) pin
void RFM69Base::setCS(uint8_t newSPISlaveSelect) {
...
uint8_t RFM69Base::readTemperature(uint8_t calFactor) // returns centigrade
{
  setMode(RF69_MODE_STANDBY);
...
void RFM69Base::rcCalibration()
{
  writeReg(REG_OSC1, RF_OSC1_RCCAL_START);
  while ((readReg(REG_OSC1) & RF_OSC1_RCCAL_DONE) == 0x00);
}

inline void RFM69Base::maybeInterrupts()
{
